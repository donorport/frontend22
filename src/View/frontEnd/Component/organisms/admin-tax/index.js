...
const AdminTax = () => {
  ...
  const getTaxList = async (page, field, type, year) => {
    ...
    console.log('~~ getTaxList end fetch:', {
      now: Date.now(),
      difference: Date.now() - LOGGING_time
    });
    console.log(
      'should have taxListResponse.data.success && taxListResponse.data = {data, totalPages, totalRecord, ...}:',
      { taxListResponse }
    );
